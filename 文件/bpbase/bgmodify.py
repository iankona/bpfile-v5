...
class 类(backindex.类):
    ...
    def __readprenone__(self, bnode, prebyte, pretype, containbyte, containtype):
    ...
    def __readpretype__(self, bnode, prebyte, pretype, containbyte, containtype):
        size1 = backend.getpretypestep(pretype)
        prebyte, containbyte = bnode.byte[0:size1], bnode.byte[size1:]
        bnode.prebyte = prebyte
        bnode.pretype = pretype
        bnode.prevalue = backend.cast(prebyte, pretype, bnode.byteorder)
    ...
    def 类实例(self, size, name, i, j, pretype="", containtype=""):
        ...
        prebyte, containbyte = b"", bnode.byte 
        if pretype == "":
            self.__readprenone__(bnode, prebyte, pretype, containbyte, containtype)
        else:
            self.__readpretype__(bnode, prebyte, pretype, containbyte, containtype)
